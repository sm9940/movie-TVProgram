;(function () {
         ...
         for (const result of results) {
            ...
            $('.mySwiper > .images')
               ...
               result.id
                    ...
                    <img src="${imgURL}"alt="영화포스터" height="100"/>
                    </a>
                    <h3>${result.title}</h3>
   ...
   $.ajax({
      ...
      success: function (data) {
         ...
         if (data.poster_path === null) {
            $('.getDetailMovies > .getDetailMoviesName > .row').append(`
         ...
         <div style= "display:flex;justify-content: center; flex-direction:column;"><h2 style="font-size: 2rem; font-weight: bolder">${
            ...
            <p style ="font-size: 1.2rem"> 개요:${data.overview}<p/>
            <p style="color:red">★ 평점: ${data.vote_average.toFixed(2)}<p/>
            <p>상영일: ${data.release_date.replaceAll('-', '.')}</p>
            <p class="genre">장르:
            </div>
            </div>
            `)
   ...
   //주요인물
   let peopleURL = `https://api.themoviedb.org/3/movie/${id}/credits?language=ko-KR`
   ...
   $.ajax({
      ...
      `)
                  ...
                  $('.PeopleList > .row > .container')
                     .append(`<div class="card bg-dark " style="width:18rem">
   ...
   `)
            ...
            $('.PeopleList > .row > .container')
               .append(`<div style="margin-right:20px;">
                         <p>주요 인물이 없습니다</p>
                         </div>
                         `)
         }
